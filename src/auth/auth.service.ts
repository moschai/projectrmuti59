...
export class AuthService {
    ...
    async adminAuth(adminAuthDto: AdminAuthDto): Promise<any> {
        const adminResponse = await this.adminRepo.validateAdminPassword(adminAuthDto);
        if (adminResponse) {
    ...
    async authorityAuth(authorityAuthDto: AuthorityAuthDto): Promise<any> {
        const authorityResponse = await this.authorityRepo.validateAuthorityPassword(authorityAuthDto);
