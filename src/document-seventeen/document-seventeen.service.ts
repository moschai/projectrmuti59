...
export class DocumentSeventeenService {
  ...
  async createDocument(createDocumentSeventeenDto: CreateDocumentSeventeenDto) {
    const teacherteath = await this.authorityRepo.findOne({
      where: { id_authority: createDocumentSeventeenDto.teacherteath_id },
    });
    if (!teacherteath) {
    ...
    await this.docuemntSeventeenTableRepo.createTableSeventeen(
      createDocumentSeventeenDto,
      documentSeventeen
    );
