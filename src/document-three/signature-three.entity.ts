import { Entity, BaseEntity, PrimaryGeneratedColumn, OneToOne, ManyToOne, JoinColumn, Column, CreateDateColumn, UpdateDateColumn } from "typeorm";
...
@Entity()
export class fte_signature_three extends BaseEntity {

    @PrimaryGeneratedColumn()
    idsig: number;


    @OneToOne(type => fte_document_type_three, three => three.signature, { eager: false })
    type: fte_document_type_three;
    ...
    head_service_or_deanoffice_leave: string;
    //ชำระเงินค่าลงทะเบียน
    @Column({ type: 'tinyint', width: 3, nullable: true })
    head_service_or_deanoffice_payregister: number;
    //ชำระเงินค่ารักษาสภาพ
    @Column({ type: 'tinyint', width: 3, nullable: true })
    head_service_or_deanoffice_paycurestd: number;
