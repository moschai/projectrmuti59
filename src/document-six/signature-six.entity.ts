...
@Entity()
export class fte_signature_six extends BaseEntity {

    @PrimaryGeneratedColumn()
    idsig: number;

    @OneToOne(type => fte_document_type_six, six => six.signature, { eager: false })
    ...
    //ลงทะเบียนเพิ่มครั้งแรก
    @Column({ type: 'varchar', length: 128, nullable: true })
    head_service_or_deanoffice_addregisterfirst: string;
    //เคยลงเพิ่มแล้ว เทมอสุดท้ายจะจบ
    @Column({ type: 'tinyint', width: 3, nullable: true })
    head_service_or_deanoffice_registernever: number;
    //เพิ่มแล้ว แต่ยังไม่ชำระเงิน
    @Column({ type: 'tinyint', width: 3, nullable: true })
    head_service_or_deanoffice_addregisternotpay: number;
    //ยังไม่ดำเนินการถอนรายวิชาในระบบ
    @Column({ type: 'tinyint', width: 3, nullable: true })
    head_service_or_deanoffice_donotinformation: number;
