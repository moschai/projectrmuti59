...
export class fte_signature_fifteen extends BaseEntity {
    ...
    @OneToOne(type => fte_document_type_fifteen, fifteen => fifteen.signature, { eager: false })
    ...
    std_pass_activity: number;
    //ไม่ผ่านกิจกรรม แต่เข้าร่วมได้เทมอนี้
    @Column({ type: 'tinyint', width: 3, nullable: true })
    std_notpass_activity_register: number;
    //ไม่ผ่านกิจกรรม และไม่สามารถเข้าร่วมได้เทมอนี้
    @Column({ type: 'tinyint', width: 3, nullable: true })
    std_notpass_activity_notregister: number;
    //อื่นๆ
    @Column({ type: 'tinyint', width: 3, nullable: true })
    ...
    //status การเซ็น (เซ็นแล้ว ยังไม่เซ็น)
    @Column({ type: 'tinyint' })
    hauthority_activity_status_sig: SignatureStatus;
