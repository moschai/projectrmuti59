...
@Entity()
export class fte_signature_four extends BaseEntity {

    @PrimaryGeneratedColumn()
    idsig: number

    @OneToOne(type => fte_document_type_four, four => four.signature, { eager: false })
    type: fte_document_type_four;
    ...
    //ลงทะเบียนเกินไม่ใช่เทมอสุดท้าย
    @Column({ type: 'tinyint', width: 3, nullable: true })
    notlastterm: number;
    //ลงทะเบียนเกินเทมอสุดท้าย
    @Column({ type: 'tinyint', width: 3, nullable: true })
    lastterm: number;
    //ลงทะเบียนเกินครั้งแรก
    @Column({ type: 'tinyint', width: 3, nullable: true })
    firstregister: number;
    //ลงทะเบียนต่ำกว่าเกณฑ์
    @Column({ type: 'tinyint', width: 3, nullable: true })
    lowgister: number;
