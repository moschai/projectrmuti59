...
@Entity()
export class fte_signature_seven extends BaseEntity {

    @PrimaryGeneratedColumn()
    idsig: number;

    @OneToOne(type => fte_document_type_seven, seven => seven.signature, { eager: false })
    ...
    //ถอนรายวิชาหน่วยกิตต่ำกว่าเกณฑ์
    @Column({ type: 'tinyint', width: 3, nullable: true })
    head_service_or_deanoffice_withdeawlowunit: number;
    //ถอนแล้ว แต่ยังไม่ชำระเงิน
    @Column({ type: 'tinyint', width: 3, nullable: true })
    head_service_or_deanoffice_withdrawnotpay: number;
